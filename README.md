...
* **Bus Topology** — scan the E3 internal CAN bus topology from the browser; colour-coded table, results cached until next scan
...
The **Bus Topology** page (sidebar: *Bus Topology*) reads the E3 internal CAN bus topology directly from the browser. It collects the same data as `open3e_topology`:

* **DID 256** (`BusIdentification`) — device identity for every UDS-accessible device
* **DIDs 954, 1286–1289** (`BusTopologyMatrix*`) — the internal topology map of each device

The results are displayed as a colour-coded table showing all UDS-accessible devices and all topology elements grouped by bus type (CanInternal, CanExternal, CanRaw, ModBus, ServiceBus). Devices reachable via UDS are marked with a **UDS** badge.

Press **Scan Topology** to start a new scan. The scan runs alongside the CAN polling engine without interrupting live data collection. Results are cached server-side and displayed immediately on subsequent visits to the page until the next scan is triggered.

> **Prerequisite:** System Depiction must have been run at least once so that `devices.json` exists.
...
Use `open3e_topology` to read the E3 CAN bus topology from all known devices and
produce a structured summary. The command requires a `devices.json` file as
generated by `open3e_depictSystem`.

It reads two categories of data points from each device:

* **DID 256** (`BusIdentification`) — device identity, always present on any UDS-accessible E3 device
* **DIDs 954, 1286–1289** (`BusTopologyMatrix*`) — the device's internal topology map (only DIDs with a proper codec are read; raw-only DIDs are skipped automatically)

The result shows all UDS-accessible devices and the full internal bus topology
with deduplication across multiple reporting devices. Devices reachable via UDS
are marked with a **UDS** badge in the HTML output.

## Usage

    open3e_topology [-h] (-c IFACE | -d IP) -devices FILE [-o STEM] [--json] [-v]

    options:
      -c IFACE, --can IFACE   CAN interface, e.g. can0
      -d IP, --doip IP        DoIP IP address, e.g. 192.168.1.1
      -devices FILE           Path to devices.json (required)
      -o STEM                 Write STEM.json and STEM.html
      --json                  Print JSON to stdout instead of text table
      -v, --verbose           Show progress during data collection

## Examples

Print a text table to the terminal:

    open3e_topology -c can0 -devices devices.json

Write JSON and HTML report files:

    open3e_topology -c can0 -devices devices.json -o topology

Print JSON for use in scripts or further processing:

    open3e_topology -c can0 -devices devices.json --json | jq .udsDevices

Write report files and show progress:
...
|---|---|
| Text table | stdout by default |
| JSON | `--json` (stdout) or `-o STEM` → `STEM.json` |
| HTML page | `-o STEM` → `STEM.html` |

The HTML page is self-contained with colour-coded bus types and UDS badges.
The JSON `scanTime` is stored in ISO 8601 UTC; the HTML page displays it in
the host system's local time and locale format.
...
* **Sensor unit fixes** in `Open3Edatapoints.py`: corrected swapped `unit=` on `Average` field of DID 321 (`CompressorInletTemperatureSensor`, was `hPa` → `°C`) and DID 322 (`CompressorInletPressureSensor`, was `°C` → `hPa`); fixed DID 279/281 (`BufferMid`/`BufferTopTemperatureSensor`) where `Actual` used `desc=` instead of `unit=`
...
### 0.7.1 (2026-05-05)
* **New: `open3e_topology`** — reads E3 CAN bus topology from all known devices and produces a text table (stdout), JSON, and a colour-coded HTML report. Requires `devices.json` as generated by `open3e_depictSystem`. See [Bus Topology Analysis](#bus-topology-analysis) for details.
* Updated CI test matrix: Python 3.10–3.13 (dropped EOL 3.9, added 3.11–3.13); upgraded GitHub Actions to checkout@v4 and setup-python@v5
* Updated `black` dev dependency to `>=26.3.1` (fixes CVE-2024-21503, CVE-2026-32274)
