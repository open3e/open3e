**_New codec `O3ESwitch`_**: decodes a device/variant-dependent payload selected by a discriminator byte (e.g. ZigBee device type), with per-case sub-structure and a fallback `default` case — see discussion #369

**_New: `open3e_topology`_** — reads E3 CAN bus topology from all known devices and produces a text table (stdout), JSON, and a colour-coded HTML report. Requires `devices.json` as generated by `open3e_depictSystem`. See [Bus Topology Analysis](#bus-topology-analysis) for details.

**_New: Web UI_** — browser-based management interface. See [Web UI](#web-ui)
...
* **ZigBee DIDs 2086–2143 and 2262–2319** (all 40 standard 57-byte `*DeviceCurrentValues`): replaced flat fixed-layout codec with `O3ESwitch` matching the 68-byte variant structure; named `RSSI` (off01), added `unit=` and `decimals=2` to `SignalLevel`, per-case sub-structure for all 5 device types (Climate sensor, TRV, Floor thermostat, Actuator NC/NO)
